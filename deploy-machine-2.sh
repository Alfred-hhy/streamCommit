...
#   ./deploy-machine-2.sh <do_ip> <your_ip> <verifier_ip>
#
# 示例:
...
# 检查参数
if [ $# -ne 3 ]; then
    echo "使用方法: $0 <do_ip> <your_ip> <verifier_ip>"
    echo ""
    echo "参数说明:"
    echo "  do_ip:        DO Server 的 IP 地址"
    echo "  your_ip:      本机 IP 地址 (SS Server)"
    echo "  verifier_ip:  Verifier Server 的 IP 地址"
    echo ""
    echo "示例:"
    echo "  ./deploy-machine-2.sh 192.168.1.10 192.168.1.20 192.168.1.30"
    exit 1
fi
...
print_header "VDS SS Server (机器2) 一键部署"
...
if docker-compose ps 2>/dev/null | grep -q vds-ss-server; then
    print_step "停止旧的服务..."
...
print_step "检查 SS Server 健康状态..."

while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    if curl -s http://localhost:5002/health > /dev/null 2>&1; then
        print_info "✓ SS Server 健康检查通过"
...
print_step "测试到 Verifier Server ($VERIFIER_IP:5003) 的连接..."
if curl -s http://$VERIFIER_IP:5003/health > /dev/null 2>&1; then
    print_info "✓ Verifier Server 可访问"
else
    print_warn "⚠ Verifier Server 暂无响应（可能还未启动）"
fi
...
print_header "✅ SS Server (机器2) 部署完成！"
