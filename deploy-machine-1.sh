...
#   ./deploy-machine-1.sh <your_ip> <ss_ip> <verifier_ip>
#
# 示例:
...
# 检查参数
if [ $# -ne 3 ]; then
    echo "使用方法: $0 <your_ip> <ss_ip> <verifier_ip>"
    echo ""
    echo "参数说明:"
    echo "  your_ip:      本机 IP 地址 (DO Server)"
    echo "  ss_ip:        SS Server 的 IP 地址"
    echo "  verifier_ip:  Verifier Server 的 IP 地址"
    echo ""
    echo "示例:"
    echo "  ./deploy-machine-1.sh 192.168.1.10 192.168.1.20 192.168.1.30"
    exit 1
fi
...
print_header "VDS DO Server (机器1) 一键部署"
...
if docker-compose ps 2>/dev/null | grep -q vds-do-server; then
    print_step "停止旧的服务..."
...
print_step "检查 DO Server 健康状态..."

while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    if curl -s http://localhost:5001/health > /dev/null 2>&1; then
        print_info "✓ DO Server 健康检查通过"
...
print_header "✅ DO Server (机器1) 部署完成！"
